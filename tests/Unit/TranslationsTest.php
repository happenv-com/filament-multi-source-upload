...
it('translates every key and nothing else', function (?string $english, ?string $translation): void {
    ...
    // A value copied from English is an untranslated string, unless the text really is the same
    // in that language — list those here as "locale.key", or as "key" for every locale.
    $sameAsEnglish = ['url_placeholder', 'fil.file_tab', 'it.file_tab', 'ms.import'];
    $locale = basename(dirname($translation));

    foreach ($expected as $key => $value) {
        if (! in_array($key, $sameAsEnglish, true) && ! in_array("{$locale}.{$key}", $sameAsEnglish, true)) {
...
it('ships a translation for every locale Filament ships', function (string $locale): void {
    expect(dirname(__DIR__, 2) . "/resources/lang/{$locale}/multi-source-file-upload.php")->toBeFile();
