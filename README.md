...
- **Size cap** — the download is written straight to a temporary file and aborted the moment it exceeds `maxUrlImportSize()` (falling back to `maxSize()`, then 25 MB). A declared `Content-Length` over the cap stops it before the body arrives.
